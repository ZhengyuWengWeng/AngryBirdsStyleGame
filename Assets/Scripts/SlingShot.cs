...
public class SlingShot : MonoBehaviour
{
            ...
            case SlingshotState.UserPulling:
                ...
                if (Input.GetMouseButton(0))
                {
                    ...
                    if (Vector3.Distance(location, SlingshotMiddleVector) > 1.5f)
                    {
                        //basic vector maths :)
                        var maxPosition =
                            (location - SlingshotMiddleVector).normalized
                            * 1.5f
                            + SlingshotMiddleVector;
                    ...
                    float distance =
                        Vector3.Distance(
                            SlingshotMiddleVector,
                            BirdToThrow.transform.position
                        );
    ...
    private void ThrowBird(float distance)
    {
        //get velocity
        Vector3 velocity =
            SlingshotMiddleVector - BirdToThrow.transform.position;
        ...
        //set the velocity
        BirdToThrow.GetComponent<Rigidbody2D>().velocity =
    ...
    {
        SlingshotLineRenderer1.SetPosition(
            1,
            BirdToThrow.transform.position
        );

        SlingshotLineRenderer2.SetPosition(
            1,
            BirdToThrow.transform.position
        );
    }
    ...
    void DisplayTrajectoryLineRenderer2(float distance)
    {
        ...
        float time =
            segmentScale / segVelocity.magnitude;
